...
import { Droplets, Phone, Lock, Eye, EyeOff, ArrowRight, ArrowLeft, AlertCircle, Check } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "otp") {
      // shouldCreateUser: false — вход не должен создавать аккаунт неизвестному номеру
      ...
      setLoading(false);
      if (err) {
        if (/not.*found|no.*user|signups? not allowed|shouldCreateUser|user not found/i.test(err.message)) {
          setError("Аккаунт с этим номером не найден. Зарегистрируйтесь.");
        } else if (/sms|sending|provider|twilio/i.test(err.message)) {
          setError("Не удалось отправить SMS-код. Пока можно войти по паролю (ниже).");
        } else {
    ...
    if (!password) { setLoading(false); setError("Введите пароль"); return; }
        ...
        <div className="w-full max-w-md">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {mode === "otp" && (
                <p className="text-xs text-slate-400">Мы отправим SMS с кодом на этот номер.</p>
              )}
              ...
              <motion.button type="submit" disabled={loading || success}
                ...
                {loading
                  ? <><svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg> {mode === "otp" ? "Отправляем…" : "Входим…"}</>
                  : <>{mode === "otp" ? "Получить код" : "Войти"} <ArrowRight className="w-4 h-4" /></>
                }
              </motion.button>
              ...
              <button type="button" onClick={() => { setMode(mode === "otp" ? "password" : "otp"); setError(""); }}
                ...
                {mode === "otp" ? "Войти по паролю" : "Войти по SMS-коду"}
              </button>
            </form>
